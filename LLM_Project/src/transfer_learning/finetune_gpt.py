...
def freeze_parameters(model, freeze_embeddings=True):
    """
    Freeze parameters of the model to prevent updates during fine-tuning.
    
    Args:
        model (nn.Module): GPT model instance.
        freeze_embeddings (bool): Whether to freeze token and position embeddings.
    """
...
def finetune_one_epoch(model, data_loader, optimizer, device, ignore_index=-100):
    """
    Fine-tune the model for one epoch.

    Args:
        model (nn.Module): The GPT model.
        data_loader (DataLoader): DataLoader for the fine-tuning dataset.
...
def evaluate_model(model, data_loader, device, ignore_index=-100):
    """
    Evaluate the fine-tuned model on validation/test data.
...
def main():
    ...
    config = {
        ...
        "fine_tuned_checkpoint": "checkpoints/gpt2_finetuned.pth"
    }
    ...
    # Load datasets
    train_loader = create_dataloader_v1("data/processed/finetune_train.txt", config["batch_size"])
    val_loader = create_dataloader_v1("data/processed/finetune_val.txt", config["batch_size"])
    ...
    for epoch in range(config["num_epochs"]):
        ...
        # Fine-tuning
        train_loss = finetune_one_epoch(model, train_loader, optimizer, device)
